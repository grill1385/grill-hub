...
- Mapa no Roteiro (RouteMap): Leaflet + tiles Carto dark; geocoding das cidades via Nominatim com cache em localStorage (`grill-geo:*`); se um alojamento tiver link Google Maps com coordenadas (@lat,lng, q=, !3d!4d), usa-se essa morada; trajeto de carro via OSRM público (router.project-osrm.org), fallback linhas retas; popups com datas/noites/alojamentos. Transportes: aparecem 6 meses antes do início, prazo 3 meses antes. Alojamento: aparecem 4 meses antes, prazo 2 meses antes. Atribuição de membros persiste em `vacation_tasks` por `auto_key`; tarefas manuais também existem (done toggle).
